...
// ─── Часовой пояс для дедупликации (не отображения!) ──────────────
// APP_TZ_OFFSET выше зависит от cookie конкретного браузера — годится для
// отображения "сегодня" в UI, но НЕЛЬЗЯ использовать как границу суток при
// проверке "уже была ли запись" для записей без привязанной точки: два
// разных админа (или сессия до того, как cookie проставился) могут в этот
// момент посчитать местные сутки по-разному и либо пропустить дубликат,
// либо ложно решить, что запись "другого дня". SERVER_TZ_OFFSET — фиксированное
// серверное значение (не зависит от браузера/cookie), используемое именно
// там, где нужна детерминированная граница суток: processAccess(),
// manual_pass.php, api/bulk_pass.php, api/offline_sync.php при отсутствии
// точки питания у записи.
define('SERVER_TZ_OFFSET', env('SERVER_TZ_OFFSET', '+03:00'));
