...
/**
 * Ссылка на площадку региона.
 *
 * В реестре домен хранится без схемы («nrg.severfoods.ru»), а иногда его
 * вписывают руками уже со схемой или со слэшем на конце — приводим к одному
 * виду, иначе ссылка получится вида «https://https://…» и не откроется.
 *
 * Единого входа (SSO) между админкой и площадками нет и не планируется: у них
 * разные домены, разные сессии и принципиально разные способы входа — здесь
 * логин с паролем, там QR-код. Ссылка ведёт на страницу входа площадки, где
 * человек прикладывает карту.
 */
function adminRegionUrl(array $region, string $path = '/login.php'): string
{
    $domain = trim((string)($region['domain'] ?? ''));
    if ($domain === '') return '';
