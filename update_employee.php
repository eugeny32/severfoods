...
// Администратор работает с сотрудниками всех организаций: ограничение своей
// организацией мешало работе на площадках, где один администратор ведёт
// несколько подрядчиков.
if (!$is_super) {
    // Единственный сохранённый запрет и настоящая цель этой проверки: защита
    // от компрометации QR-кода супер-администратора. Такую карточку обычный
    // администратор не открывает и не редактирует ни при каких условиях —
    // код служит паролем.
...
// Организацию может менять и администратор: перевод сотрудника между
// подрядчиками — обычное дело на площадке.
