...
(function initKioskGestures() {
    function attach() {
        ...
        // На смарт-терминале Эвотор экранного закрепления нет вовсе: терминал
        // принадлежит кассиру, и выходить из киоска там не из чего. Вешать
        // жест, который ничего не делает, — верный способ получить жалобу
        // «нажимаю десять раз, ничего не происходит».
        if (window.SF_NO_KIOSK) return;
...
function renderSettings() {
    ...
    //
    // На смарт-терминале Эвотор карточку не показываем: обновления там ставит
    // Эвотор.Маркет, а кнопка «Проверить сейчас», которая ничего не может
    // найти, хуже её отсутствия. Вместо неё — строка о том, откуда приходят
    // обновления, чтобы вопрос «а как обновиться» не оставался без ответа.
    if (window.SF_UPDATES_DISABLED) {
        ...
        <p class="setting-note">Приложение обновляется через Эвотор.Маркет: новая версия
        устанавливается на терминал сама, как и остальные приложения Эвотора.</p>
    `);
    } else {
    grid.innerHTML += card('Обновления', 'cloud-download-alt', `
        <div class="setting-row"><label>Текущая версия</label><span id="updCurVer">—</span></div>
        ...
        <p class="setting-note">Обновление скачивается автоматически в фоне при наличии интернета и устанавливается само ночью (01:00–05:00 по местному времени точки), когда компьютер к тому же простаивает 5+ минут — чтобы точно не прервать сканирование. Кнопка «Установить сейчас» перезапускает приложение немедленно, в любое время.</p>
    `);
    }
