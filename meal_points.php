...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add') {
        ...
        if ($tz !== '' && !preg_match('/^[+-]\d{2}:\d{2}$/', $tz)) {
            ...
            $oldTz = $pdo->prepare("SELECT tz_offset FROM meal_points WHERE id=?");
            $oldTz->execute([$pid]);
            $oldTzVal = $oldTz->fetchColumn();

            $pdo->prepare("UPDATE meal_points SET tz_offset=? WHERE id=?")->execute([$tz === '' ? null : $tz, $pid]);
            logAction('set_point_tz', "Точка ID:{$pid} → часовой пояс " . ($tz !== '' ? $tz : 'по умолчанию') . " (было: " . ($oldTzVal ?: 'по умолчанию') . ")");

            // Смена часового пояса меняет "местные сутки" для ВСЕХ уже существующих
            // записей этой точки (местная дата пересчитывается на лету из UTC при
            // каждом запросе) — это может повлиять на дедупликацию/отчёты по старым
            // записям, а не только на новые. Предупреждаем явно, а не молча меняем.
            $msg = "Часовой пояс точки обновлён. Внимание: это меняет группировку по местным суткам "
                 . "и для УЖЕ СУЩЕСТВУЮЩИХ записей этой точки — если после смены появились "
                 . "подозрительные повторы, проверьте раздел «Обслуживание БД» → «Найти дубликаты».";
        }
