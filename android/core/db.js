 ...
 * База Android-версии.
 *
 * Схема и все запросы — построчный порт offline/src/db.js. Логика дедупликации
 * (hasTodayLog + окно суток по часовому поясу точки) сохранена дословно: это
 * самое ценное, что нельзя потерять при переносе.
 *
 * Отличия от Windows-версии — только в способе хранения:
 *   - файл в %APPDATA%  →  запись в IndexedDB (см. storage.js);
 *   - синхронный fs.writeFileSync  →  асинхронная запись, поэтому появились
 *     saveNow()/saveSoon() вместо одного _save().
 */
(function (global) {
    'use strict';
    ...
    // ── сохранение ────────────────────────────────────────────
    // Планшет засыпает и может быть выключен в любой момент, поэтому запись
    // прохода сотрудника сохраняется НЕМЕДЛЕННО (saveNow), без задержки — как
    // и в Windows-версии. Отложенное сохранение (saveSoon) применяется только
    // к массовым операциям синхронизации справочника.
    ...
    async function saveNow() {
        ...
        // Последовательная очередь: параллельные записи одного и того же
        // образа могут переставиться местами и затереть более свежий.
        _saving = (_saving || Promise.resolve())
            .catch(() => {})
            .then(() => SFStorage.save(_export()));
        return _saving;
    }
    ...
    function endBatch() {
        ...
        if (_batchDepth === 0 && _batchDirty) { _batchDirty = false; saveNow(); }
    }
    ...
    async function init() {
        ...
        const bytes = await SFStorage.load();
    ...
    function _get(sql, params = []) { return _all(sql, params)[0] || null; }

    function _run(sql, params = []) { db.run(sql, params); saveSoon(); }
    ...
    function upsertMealPoint(p, schedules) {
        db.run(`INSERT INTO meal_points (id, point_name, point_code, city, address, is_active)
                VALUES (?,?,?,?,?,?)
                ON CONFLICT(id) DO UPDATE SET
    ...
    /** Проход сотрудника. Сохраняется немедленно — планшет могут выключить. */
    function insertMealLog(log) {
        db.run(`INSERT INTO meal_logs
    ...
    function setMeta(key, value) {
        db.run('INSERT INTO sync_meta (key,value) VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value',
            [key, value === null ? null : String(value)]);
        return saveNow(); // сессия и токены не должны теряться при выключении
    }

    global.SFDb = {
        init, saveNow, beginBatch, endBatch,
