...
function doPush(): void
{
    ...
    // Одна порция за запрос. Старые версии приложения (1.3.x) шлют ВСЕ
    // накопленные записи одним запросом и ждут ответа всего 15 секунд: точка,
    // простоявшая без связи несколько дней, не укладывалась в таймаут, из-за
    // чего НИ ОДНА запись не помечалась отправленной и на следующем часу
    // уходил тот же, уже больший пакет — точка застревала навсегда.
    //
    // Ограничение безопасно для всех версий: клиент помечает отправленными
    // только те записи, что вернулись в results (см. markLogsSynced в
    // offline/src/db.js), а остаток спокойно уйдёт следующей синхронизацией.
    // 500 подобрано под самый жёсткий случай — те самые 15 секунд у 1.3.x.
    // После снятия ALTER TABLE с горячего пути на запись приходится четыре
    // обращения к базе, то есть даже на медленном хостинге порция укладывается
    // с запасом. Новые версии шлют по 200 и в лимит не упираются вовсе.
    $maxRecords = 500;

    $all       = $body['records'];
    $records   = array_slice($all, 0, $maxRecords);
    $remaining = max(0, count($all) - count($records));

    // Порция всё равно может оказаться дольше стандартного лимита PHP на
    // медленной базе — обрыв на середине оставил бы часть записей вставленными,
    // но без ответа клиенту, и он прислал бы их снова.
    @set_time_limit(0);
    ...
    echo json_encode([
        ...
        'errors'   => $errors,
        'results'  => $results,
        // Сколько записей из присланного пакета осталось на следующий раз.
        // Поле добавлено, а не заменяет существующие — старые версии его
        // просто не читают.
        'remaining' => $remaining,
        'ts'       => time(),
    ]);
