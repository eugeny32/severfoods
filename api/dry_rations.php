...
if ($method === 'POST') {
    ...
    // Count existing in stats period
    ...
    // Count new dates that fall within stats period
    $newInPeriod = 0;
    foreach ($dates as $d) {
        if ($d >= $statsFrom && $d <= $statsTo) $newInPeriod++;
    }
    if ($existing + $newInPeriod > 5) {
        echo json_encode(['ok'=>false,'error'=>'Превышен лимит 5 дней за период (уже: '.$existing.', добавляется: '.$newInPeriod.')']); exit;
    }

    $inserted = 0;
    $skipped  = 0;
    $stmt = $pdo->prepare("INSERT IGNORE INTO dry_rations (employee_id, ration_date, ration_type, created_by) VALUES (?,?,?,?)");
    foreach ($dates as $d) {
